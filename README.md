...
1. 由实时区间构建前驱关系（`respond(A) <= invoke(B)` ⇒ `A ≺ B`），位掩码 Floyd-Warshall 求传递闭包（同时得到逆关系，用于自同构分类）；若成环（如两个同时刻的瞬时操作互相前驱）直接判无解；
2. 深度优先搜索拓扑序空间：每个操作都是独立搜索边——同值并发写绝不能合并成一步，因为其中一次写入可能要在 CAS 之后“恢复”值（同值写在 CAS 前后语义角色不同）；候选按标识字典序展开，因此**首个**完整解即为字典序最小全序；
3. 两类可靠剪枝保证 24 个操作的对抗性实例也在毫秒级完成：
   - **自同构操作**：寄存器行为相同（同值写 / 同值读 / 相同 expected-update-success 的 CAS）且前驱、后继集合也相同的操作可互换标签，只展开当前可用成员中标识最小者；
   - **空闲无副作用操作**：当前值下合法且不改变值的操作（读到当前值的读、失败 CAS、`update == expected` 的成功 CAS）只展开最小者（写入永不享受此剪枝——写入当前值与未来值变更冲突）；
   - 另有状态层的前向可行性松弛：逐一检查剩余读/CAS 是否仍可能观察到可接受值（实时约束 + 安装者可达性），并按值统计“进入纪元”（写或跨值 CAS）与“退出纪元”（破坏性 CAS）的供需；
4. 被剪枝代表的分支若通向解，必然存在与之不同的标签互换/无副作用换位解，该多重性一路传播到叶节点，据此精确判定唯一性（找到 2 个不同解即停止）；无解时不返回任何部分顺序。死端状态 `(剩余操作掩码, 寄存器值)` 记忆化。
...
tests/       # 求解器语义测试（含 20–24 操作边界历史）+ API 校验与 HTTP 冒烟
